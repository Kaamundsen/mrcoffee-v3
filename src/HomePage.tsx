    ...
    <>
      ...
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between overflow-visible px-6">
          <Link to="/" className="block min-w-0 max-w-[calc(100vw-5.5rem)] shrink sm:max-w-none">
            <img
              src={
                navSolid
              ...
              className={`w-auto max-w-full object-contain object-left transition-[height,transform] duration-300 ${
                navSolid ? 'h-[3.25rem] sm:h-[3.717rem]' : 'h-[3.35rem] sm:h-[4.213rem] -translate-y-[3px]'
              }`}
            />
          </Link>
        ...
        {mobileOpen && (
          ...
          >
            ...
            <div className="mt-8 flex flex-col gap-6">
              <Link to="/" className="text-2xl font-serif text-white hover:text-antique-brass" onClick={() => setMobileOpen(false)}>
                Hjem
              </Link>
              <div>
                <button
...
export const Footer = () => (
  <footer className="border-t border-white/10 bg-chinese-black py-12">
    <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-8">
      ...
      />
      <span className="text-white/30 text-xs uppercase tracking-normal">
        {footer.copyright}{' '}
